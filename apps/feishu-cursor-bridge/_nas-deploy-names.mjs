...
console.log("[stop]", JSON.stringify(await project("stop")).slice(0, 140));
await new Promise((r) => setTimeout(r, 1500));
console.log("[start]", JSON.stringify(await project("start")).slice(0, 140));
...
// Re-backfill to attach names from message.list sender.name
const bf = await (
  await fetch(
...
const people = (list.threads || [])
  .filter((t) => t.chatType === "p2p" || t.chatType === "group")
  .slice(0, 12)
  .map((t) => ({
    chatType: t.chatType,
    name: t.senderName || null,
    openId: t.senderOpenId ? String(t.senderOpenId).slice(0, 10) + "…" : null,
  }));

const status = await (
  await fetch(
    `http://192.168.1.82:8787/admin/log-status?probe=0&token=${encodeURIComponent(token)}`,
  )
).json();
  ...
  JSON.stringify(
    {
      ...
      people,
      namesCache: status.names,
      columns: ["最近活动", "类型", "姓名", "条数", "预览"],
    },
    null,
    2,
  ),
...
const named = people.filter((p) => p.name && p.name !== "（未知）" && p.name !== "机器人");
process.exit(named.length > 0 || (status.names?.cachedCount || 0) > 0 ? 0 : 2);
