...
| E 检测应用 | 可视化检测 UI / 演示 | **MVP**：列表/详情/复核/导出；mock 可演示 |
...
| ID | 事项 | 状态 | 优先级 | 触发 |
|----|------|------|--------|------|
| R-E01 | `apps/detector-ui/` 范围说明（与 BI 分离） | 完成 | P1 | M3 前后 |
| R-E02 | 图像回放 + 框/掩码叠加 | 完成（mock） | P2 | 有样本 |
| R-E03 | 报警/缺陷列表与导出 | 完成（CSV/JSON） | P2 | M4 |
| R-E04 | 推理服务接口草稿 | 暂缓 | P2 | 有模型；本期为文件快照 + `/api/review` |
